...
public class HookService
{
	...
	/// <summary>
	/// Constructs a new HookService with a logger
	/// </summary>
	...
	internal void InitialiseAPI()
	{
		try
		{
			_logger.LogInformation("TerrariaAPI starting (Protocol {terrariaVersion} ({terrariaRelease}), OTAPI {otapiVersion})",
				Main.versionNumber2,
				Main.curRelease,
		...
		{
			_logger.LogError(ex, "Startup aborted due to an exception in the Server API initialization");
		}
	}

	internal void AttachOtapiHooks(string[] args)
	{
