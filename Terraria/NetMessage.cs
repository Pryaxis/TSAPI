	...
	public class NetMessage
	{
			...
			int number7 = 0)
		{
					...
					ref number2, ref number3, ref number4, ref number5))
			{
				...
				lock (NetMessage.buffer[num])
				{
						...
						case 10:
							{
								int num3 = NetMessage.CompressTileBlock(number, (int)number2, (short)number3, (short)number4,
									NetMessage.buffer[num].writeBuffer, (int)writer.BaseStream.Position);
					...
					if (Main.netMode == 1)
					{
						...
						try
						{
							...
							Main.txMsg++;
							Main.txData += num19;
							Netplay.Connection.Socket.AsyncSend(NetMessage.buffer[num].writeBuffer, 0, num19,
							...
							for (int num20 = 0; num20 < 256; num20++)
							{
								if (num20 != ignoreClient && NetMessage.buffer[num20].broadcast && Netplay.Clients[num20].Socket.IsConnected())
								{
									try
									{
										...
										Main.txMsg++;
										Main.txData += num19;
										Netplay.Clients[num20].Socket.AsyncSend(NetMessage.buffer[num].writeBuffer, 0, num19,
							...
							for (int num21 = 0; num21 < 256; num21++)
							{
									...
									Netplay.Clients[num21].SectionRange(number, (int)number2, (int)number3))
								{
									try
									{
										...
										Main.txMsg++;
										Main.txData += num19;
										Netplay.Clients[num21].Socket.AsyncSend(NetMessage.buffer[num].writeBuffer, 0, num19,
								...
								if (num22 != ignoreClient && NetMessage.buffer[num22].broadcast && Netplay.Clients[num22].Socket.IsConnected())
								{
									...
									if (flag3)
									{
										try
										{
											...
											Main.txMsg++;
											Main.txData += num19;
											Netplay.Clients[num22].Socket.AsyncSend(NetMessage.buffer[num].writeBuffer, 0, num19,
								...
								if (num23 != ignoreClient && NetMessage.buffer[num23].broadcast && Netplay.Clients[num23].Socket.IsConnected())
								{
									...
									if (flag4)
									{
										try
										{
											...
											Main.txMsg++;
											Main.txData += num19;
											Netplay.Clients[num23].Socket.AsyncSend(NetMessage.buffer[num].writeBuffer, 0, num19,
							...
							for (int num24 = 0; num24 < 256; num24++)
							{
								if (num24 != ignoreClient && NetMessage.buffer[num24].broadcast && Netplay.Clients[num24].Socket.IsConnected())
								{
									try
									{
										...
										Main.txMsg++;
										Main.txData += num19;
										Netplay.Clients[num24].Socket.AsyncSend(NetMessage.buffer[num].writeBuffer, 0, num19,
								...
								if (num25 != ignoreClient && NetMessage.buffer[num25].broadcast && Netplay.Clients[num25].Socket.IsConnected())
								{
									...
									if (flag5)
									{
										try
										{
											...
											Main.txMsg++;
											Main.txData += num19;
											Netplay.Clients[num25].Socket.AsyncSend(NetMessage.buffer[num].writeBuffer, 0, num19,
							...
							for (int num26 = 0; num26 < 256; num26++)
							{
									...
									Netplay.Clients[num26].Socket.IsConnected())
								{
									try
									{
										...
										Main.txMsg++;
										Main.txData += num19;
										Netplay.Clients[num26].Socket.AsyncSend(NetMessage.buffer[num].writeBuffer, 0, num19,
					...
					else if (Netplay.Clients[remoteClient].Socket.IsConnected())
					{
						try
						{
							...
							Main.txMsg++;
							Main.txData += num19;
							Netplay.Clients[remoteClient].Socket.AsyncSend(NetMessage.buffer[num].writeBuffer, 0, num19,
				...
				sw.Stop();
				Trace.WriteLineIf(sw.ElapsedMilliseconds > 0, string.Format("SendData lock time: {0}ms MsgID {1}", sw.ElapsedMilliseconds, (PacketTypes)msgType));
			}
		}
