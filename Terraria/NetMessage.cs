			...
			lock (NetMessage.buffer[num])
			{
				...
				if (remoteClient == -1)
				{
					if (msgType == 20)
					{
						...
						{
							if (num16 != ignoreClient && (NetMessage.buffer[num16].broadcast || (Netplay.serverSock[num16].state >= 3 && msgType == 10)) && Netplay.serverSock[num16].tcpClient.Connected && Netplay.serverSock[num16].SectionRange(number, (int)number2, (int)number3))
							{
								try
								{
						...
						for (int num17 = 0; num17 < 256; num17++)
						{
							if (num17 != ignoreClient && (NetMessage.buffer[num17].broadcast || (Netplay.serverSock[num17].state >= 3 && msgType == 10)) && Netplay.serverSock[num17].tcpClient.Connected)
							{
								...
								else
								{
									...
									{
										Rectangle rectangle = new Rectangle((int)Main.player[num17].position.X, (int)Main.player[num17].position.Y, Main.player[num17].width, Main.player[num17].height);
					...
					else
					{
						if (msgType == 28)
						{
							for (int num18 = 0; num18 < 256; num18++)
							{
								if (num18 != ignoreClient && (NetMessage.buffer[num18].broadcast || (Netplay.serverSock[num18].state >= 3 && msgType == 10)) && Netplay.serverSock[num18].tcpClient.Connected)
								{
									...
									{
										Rectangle rectangle2 = new Rectangle((int)Main.player[num18].position.X, (int)Main.player[num18].position.Y, Main.player[num18].width, Main.player[num18].height);
						...
						else
						{
							if (msgType == 27)
							{
								...
								{
									if (num20 != ignoreClient && (NetMessage.buffer[num20].broadcast || (Netplay.serverSock[num20].state >= 3 && msgType == 10)) && Netplay.serverSock[num20].tcpClient.Connected)
									{
										bool flag5 = false;
										...
										{
											Rectangle rectangle3 = new Rectangle((int)Main.player[num20].position.X, (int)Main.player[num20].position.Y, Main.player[num20].width, Main.player[num20].height);
										...
										if (flag5)
										{
											try
											{
												...
												Main.txMsgType[msgType]++;
												Main.txDataType[msgType] += num2;
												Netplay.serverSock[num20].networkStream.BeginWrite(NetMessage.buffer[num].writeBuffer, 0, num2, new AsyncCallback(Netplay.serverSock[num20].ServerWriteCallBack), Netplay.serverSock[num20].networkStream);
											}
											catch
											{
											}
							...
							{
								if (num21 != ignoreClient && (NetMessage.buffer[num21].broadcast || (Netplay.serverSock[num21].state >= 3 && msgType == 10)) && Netplay.serverSock[num21].tcpClient.Connected)
								{
									try
									{
										...
										Main.txMsgType[msgType]++;
										Main.txDataType[msgType] += num2;
										Netplay.serverSock[num21].networkStream.BeginWrite(NetMessage.buffer[num].writeBuffer, 0, num2, new AsyncCallback(Netplay.serverSock[num21].ServerWriteCallBack), Netplay.serverSock[num21].networkStream);
									}
									catch
									{
									}
