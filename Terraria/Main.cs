		...
		protected void Update()
		{
			...
			{/*
				...
				if (Main.fpsTimer.ElapsedMilliseconds >= 1000L)
				{
					...
					int arg_4CC_0 = Main.netMode;
				}
				if (Main.fixedTiming)
				{
					float num = 16f;
					float num2 = (float)Main.updateTimer.ElapsedMilliseconds;
					if (num2 + Main.uCarry < num && !Main.superFast)
					{
						Main.drawSkip = true;
						return;
					}
				...
				Main.numDust = (int)(6000f * (Main.gfxQuality * 0.9f + 0.1f));
				...
				if (Main.chatMode)
				{
					...
					while (Main.fontMouseText.MeasureString(Main.chatText).X > (float)num6)
					{
