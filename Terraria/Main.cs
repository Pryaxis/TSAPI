...
namespace Terraria
{
    public class Main
    {
        ...
        public static bool invasionDefeat;
        ...
        protected void OpenRecent()
        {
            try
            {
                ...
                {
                    using (var fileStream = new FileStream(SavePath + "\\servers.dat", FileMode.Open))
                    {
                        using (var binaryReader = new BinaryReader(fileStream))
                        {
                            binaryReader.ReadInt32();
        ...
        public static void SaveRecent()
        {
            ...
            try
            {
                using (var fileStream = new FileStream(SavePath + "\\servers.dat", FileMode.Create))
                {
        ...
        {
            if (tile[x, y].active && tileSolid[tile[x, y].type] && !tileSolidTop[tile[x, y].type] && tile[x, y].type != 10 && tile[x, y].type != 138)
            {
        ...
        private static void UpdateInvasion()
        {
            if (invasionDefeat)
            {
                ...
                for (int i = 0; i < 200; i++)
                {
                    if (npc[i].active && ((npc[i].type >= 26 && npc[i].type <= 29) || npc[i].type == 111))
                    {
                        invasionDefeat = true;
                    }
                }
        ...
        private static void UpdateClient()
        {
            ...
            if (Math.IEEERemainder(netPlayCounter, 300.0) == 0.0)
            {
                ...
                NetMessage.SendData(36, -1, -1, "", myPlayer, 0f, 0f, 0f, 0);
            }
            if (Math.IEEERemainder(netPlayCounter, 600.0) == 0.0)
            {
                NetMessage.SendData(16, -1, -1, "", myPlayer, 0f, 0f, 0f, 0);
                ...
                if (netMode != 1)
                {
                    ...
                    if (checkForSpawns >= 7200)
                    {
                        ...
                        if (WorldGen.spawnNPC == 0)
                        {
                            ...
                            {
                                if (player[num17].active)
                                {
                                    for (int num18 = 0; num18 < 48; num18++)
                                    {
                                        if (player[num17].inventory[num18] != null & player[num17].inventory[num18].stack > 0)
                                        {
                                            if (player[num17].inventory[num18].type == 71)
                                            {
                                                num15 += player[num17].inventory[num18].stack;
                                            }
                                            if (player[num17].inventory[num18].type == 72)
                                            {
                                                num15 += player[num17].inventory[num18].stack*100;
                                            }
                                            if (player[num17].inventory[num18].type == 73)
                                            {
                                                num15 += player[num17].inventory[num18].stack*10000;
                                            }
                                            if (player[num17].inventory[num18].type == 74)
                                            {
                                                num15 += player[num17].inventory[num18].stack*1000000;
                                            }
                                            if (player[num17].inventory[num18].ammo == 14 || player[num17].inventory[num18].useAmmo == 14)
                                            {
                                                flag3 = true;
                                            }
                                            if (player[num17].inventory[num18].type == 166 || player[num17].inventory[num18].type == 167 || player[num17].inventory[num18].type == 168 || player[num17].inventory[num18].type == 235)
                                            {
                                                flag4 = true;
                                            }
                                        }
                                    }
                                    int num19 = player[num17].statLifeMax/20;
                                    if (num19 > 5)
                                    {
                                        flag2 = true;
                                    }
                            ...
                            {
                                int num20 = NPC.NewNPC(dungeonX*16 + 8, dungeonY*16, 37, 0);
                                npc[num20].homeless = false;
                                npc[num20].homeTileX = dungeonX;
                                npc[num20].homeTileY = dungeonY;
                            }
                            if (WorldGen.spawnNPC == 0 && num7 < 1)
                            {
                                WorldGen.spawnNPC = 22;
                            }
                            if (WorldGen.spawnNPC == 0 && num15 > 5000.0 && num3 < 1)
                            {
